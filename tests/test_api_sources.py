...
def test_consulta_deve_retornar_status_code_204_response_vazio(client, token):
    response = client.get(url_for("api.get_sources", headers=token))
    assert response.status_code == 204


def test_para_inserir_sources_deve_retornar_status_code_201(client, token):
    data = {
        "id": 1,
        "description": "teste",
        "id_user": 1,
